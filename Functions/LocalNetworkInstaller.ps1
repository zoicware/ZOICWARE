...
try {
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        if ($checkbox2.Checked) {
            ...
            $output = pnputil /add-driver $path /subdirs /install | Select-String 'Published Name:'
            #update connected devices
            pnputil.exe /scan-devices | Out-Null
            #add all installed infs to array
            $infs = @() 
            foreach ($line in $output) {
                $infs += ($line -split ':')[1].Trim()
            }
            #convert pnputil output to object in order to find network inf name
            $devices = ((pnputil.exe /enum-devices /connected /class Net | 
            ...
            #get the inf name of connected device
            foreach ($device in $devices) {
                if ($device.ManufacturerName -eq 'Realtek') {
                    $infKeep = $device.DriverName
                    break
                }
            }
            #add infs not in use to array
            $infsRemove = @()
            foreach ($inf in $infs) {
                if ($inf -eq $infKeep) {
                    $infsRemove += $null 
            ...
            #cleanup extra installed drivers
            Write-Host 'Cleaning up Leftover Drivers...'
            foreach ($inf in $infsRemove) {
                pnputil.exe /delete-driver $inf /uninstall | Out-Null
            }
    ...
    if ($checkbox3.Checked) {
        ...
        #get the inf name of connected device
        foreach ($device in $devices) {
            if ($device.ManufacturerName -eq 'Intel') {
                $infKeep = $device.DriverName
                break
            }
        }
