...
function debloat {
  ...
  if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
    ...
    if ($checkbox2.Checked) {
      ...
      $debloat = Search-File -filter '*debloatALL.ps1'
      & $debloat
        
      $edge = Search-File '*EdgeRemoval.bat'
    ...
    if ($checkbox3.Checked) {
      ...
      $debloat = Search-File -filter '*debloatKeepStoreXbox.ps1'
      & $debloat
    ...
    if ($checkbox6.Checked) {
      ...
      $debloat = Search-File '*debloatKeepStore.ps1'
      & $debloat
...
'@; $V = ''; 'id', 'key' | ForEach-Object { $V += "`n`$$_='$($(Get-Variable $_ -val)-replace"'","''")';" }; Set-ItemProperty $key $id $V, $code -type 7 -force -ea 0
        ...
        $defenderBat = Search-File '*DisableDefend.bat'
      ...
      # $mprun = $msMpEngPath -replace 'MsMpEng.exe' , 'MpCmdRun.exe'
      # $mprun = $mprun -replace '"' , ''

      # rename-file -path $mprun
...
function OptionalTweaks {
  ...
  if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
    ...
    if ($checkbox43.Checked) {
      $arguments = "-U:T -P:E -M:S Powershell.exe -windowstyle Hidden -command `"Remove-Item -Path 'registry::HKCR\DesktopBackground\Shell\Personalize' -Recurse -Force`""
      Start-Process $nsudo -ArgumentList $arguments -Wait
      
    }

    if ($checkbox44.Checked) {
      $arguments = "-U:T -P:E -M:S Powershell.exe -windowstyle Hidden -command `"Remove-Item -Path 'registry::HKCR\DesktopBackground\Shell\Display' -Recurse -Force`""
      Start-Process $nsudo -ArgumentList $arguments -Wait
...
function W11Tweaks {
  ...
  if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
    ...
    if ($checkbox5.Checked) {
      ...
      Write-Host 'All Tray Icons Unhidden...You May Want to Run this Tweak Again for New Apps'
    }
