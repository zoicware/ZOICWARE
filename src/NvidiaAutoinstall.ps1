...
if (!(Check-Internet)) {
    ...
    if (!(Test-Path "$folder\DDU v18.0.8.6\Display Driver Uninstaller.exe")) {
        ...
        switch ($choice) {
            'OK' {
                ...
                Start-Process -FilePath $archiverProgram -NoNewWindow -ArgumentList "x -bso0 -bsp1 -bse1 -aoa `"$env:TEMP\DDU.exe`" -o`"$folder`"" -Wait
            }
        }
    }
    else {
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        if (!$appx) {
            ...
            Remove-Item "$env:TEMP\NVIDIACorp.NVIDIAControlPanel*" -Force -Recurse -ErrorAction SilentlyContinue
            $downloadedFiles = Download-AppxPackage -PackageFamilyName 'NVIDIACorp.NVIDIAControlPanel_56jybvy8sckqj' -outputDir "$env:TEMP" 
