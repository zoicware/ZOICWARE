...
function Create-Nip {
    param (
        [string]$nipPath,
        [array]$settingsInfo #array of objects containing settingId, settingValue, valueType and sometimes SettingNameInfo
    )
...
'@
    New-Item $nipPath -Force -Value $defaultContent | Out-Null
    [xml]$nipContent = Get-Content $nipPath
    $settings = $nipContent.SelectSingleNode('//Profile/Settings')
    foreach ($setting in $settingsInfo) {
...
'@
        ...
        $bgImage.Source = New-Object System.Windows.Media.Imaging.BitmapImage(
            (New-Object Uri($SplashImagePath, [UriKind]::Absolute))
        )
        ...
        $proc = Start-Process "$env:USERPROFILE\AppData\Local\Temp\NVCleanstall\setup.exe" -WorkingDirectory "$env:USERPROFILE\AppData\Local\Temp\NVCleanstall" -ArgumentList '-clean -s' -PassThru
        ...
        if ($stripDriver) {
            ...
            #check nv_dispi and also nvmdi 
            if (Test-Path "$env:windir\System32\DriverStore\FileRepository\nv_dispi*") {
            ...
            (Get-ChildItem -Path $driverPath -Directory).FullName | ForEach-Object { 
            ...
            $pluginPath = (Get-ChildItem -Path $driverPath -Directory | Select-Object -First 1).FullName + '\Display.NvContainer\plugins'
            $pluginsToRemove = @(
    ...
    $checkbox1.Text = 'Use Optimized NVCP Settings'
    ...
    $mergeLabel.ForeColor = 'White'
    $mergeLabel.Visible = $true
    $mergeLabel.BackColor = [System.Drawing.Color]::Transparent
    $mergeLabel.AutoSize = $true
    $mergeLabel.Font = New-Object System.Drawing.Font('Segoe UI', 9, [System.Drawing.FontStyle]::Bold) 
    $mergeLabel.Location = New-Object System.Drawing.Point(195, 20)
    $tabPage1.Controls.Add($mergeLabel)

    $radiobutton1 = New-Object System.Windows.Forms.checkbox
    $radiobutton1.Location = New-Object System.Drawing.Size(250, 60)
    ...
    $radiobutton2.Location = New-Object System.Drawing.Size(250, 40)
    ...
    $radiobutton3.Location = New-Object System.Drawing.Size(250, 20)
    ...
    if ($dialogResult -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        if ($radioButton1.Checked) {
            ...
            Create-Nip -nipPath "$tempDir\Custom.nip" -settingsInfo $settings

            $inspector = Search-File '*nvidiaProfileInspector.exe'
            $arguments = '-silent', '-mergeImport', "$tempDir\Custom.nip"
            & $inspector $arguments | Wait-Process
        }
            
        if ($radioButton2.Checked) {
            #enable gsync and related settings
            $settings = @()
            $settings += [PSCustomObject]@{
                settingId       = '294973784'
                settingValue    = '1'
                valueType       = 'Dword'
                settingNameInfo = 'Enable G-SYNC globally'
            }
            $settings += [PSCustomObject]@{
                settingId       = '279476652'
                settingValue    = '0'
                valueType       = 'Dword'
                settingNameInfo = 'G-SYNC'
            }
            $settings += [PSCustomObject]@{
                settingId       = '279476687'
                settingValue    = '0'
            ...
            $settings += [PSCustomObject]@{
                ...
                settingValue    = '1'
                valueType       = 'Dword'
                settingNameInfo = 'Toggle the VRR global feature'
            }
            $settings += [PSCustomObject]@{
                settingId       = '278196727'
                settingValue    = '1'
                valueType       = 'Dword'
                settingNameInfo = 'VRR requested state'
            }
            ...
            $settings += [PSCustomObject]@{
                ...
                settingValue    = '1'
                valueType       = 'Dword'
                settingNameInfo = 'Preferred refresh rate'
            }
            #set vsync to application controlled
            $settings += [PSCustomObject]@{
                settingId       = '11041231'
                settingValue    = '1620202130'
                valueType       = 'Dword'
                settingNameInfo = 'Vertical Sync'
            }
            $settings += [PSCustomObject]@{
                settingId       = '270198627'
                settingValue    = '0'
                valueType       = 'Dword'
                settingNameInfo = 'Flag to control smooth AFR behavior'
            }
            $settings += [PSCustomObject]@{
                settingId       = '5912412'
                settingValue    = '2525368439'
                valueType       = 'Dword'
                settingNameInfo = 'Vertical Sync Tear Control'
            }
            Create-Nip -nipPath "$tempDir\Custom.nip" -settingsInfo $settings
        ...
        if ($radioButton3.Checked) {
            #set dlss sr, fg and rr preset letters to recommended
            $settings = @()
            $settings += [PSCustomObject]@{
                settingId       = '283385345'
                settingValue    = '1'
                valueType       = 'Dword'
                settingNameInfo = 'Enable DLSS-SR override'
            }
            $settings += [PSCustomObject]@{
                settingId       = '6505105'
                settingValue    = '1'
                valueType       = 'Dword'
                settingNameInfo = 'DLSS Model Preset Profile'
            }
            $settings += [PSCustomObject]@{
                settingId       = '283385331'
                settingValue    = '16777215'
                valueType       = 'Dword'
                settingNameInfo = 'Override DLSS-SR presets'
            }
            $settings += [PSCustomObject]@{
                settingId       = '283385347'
                settingValue    = '1'
                valueType       = 'Dword'
                settingNameInfo = 'Enable DLSS-FG override'
            }
            $settings += [PSCustomObject]@{
                settingId       = '283385329'
                settingValue    = '16777214'
                valueType       = 'Dword'
                settingNameInfo = 'Override DLSS-FG preset'
            }
            $settings += [PSCustomObject]@{
                settingId       = '283385346'
                settingValue    = '1'
                valueType       = 'Dword'
                settingNameInfo = 'Override DLSS-RR preset'
            }
            $settings += [PSCustomObject]@{
                settingId       = '283385335'
                settingValue    = '16777215'
