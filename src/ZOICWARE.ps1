...
do {
    ...
    $restartButton = Create-ModernButton -Text 'Restart PC' -Location (New-Object Drawing.Point(($startX + $buttonWidth + $buttonSpacing), 90)) -Size (New-Object Drawing.Size($buttonWidth, $buttonHeight)) -ClickAction {
        $form.Visible = $false
        restart-pc
    } -TooltipText 'Restarts the computer.' -IconPath "$iconDir\restart.png"
    $utilitiesPanel.Controls.Add($restartButton)
