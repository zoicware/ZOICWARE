...
if ($PSScriptRoot -like '*_FOLDERMUSTBEONCDRIVE') {
    ...
    Write-Host "Searching for functions in $sysDrive" -BackgroundColor Black
            ...
            if (!(Test-Path "$folder\$latestVer" -PathType Leaf)) {
                ...
                switch ($msgBoxInput) {
                    'Yes' {
                        ...
                        # download zip
                        Get-FileFromWeb -URL $downloadUrl -File "$tempDir\zoicwareOS.zip"
            
                        Expand-Archive -Path "$tempDir\zoicwareOS.zip" -DestinationPath "$tempDir\zoicwareOS" -Force
                        Rename-Item "$tempDir\zoicwareOS" -NewName "zoicwareOS$latestVer" -Force  
                        Move-Item "$tempDir\zoicwareOS$latestVer" -Destination "$env:USERPROFILE\Desktop" -Force
                        Remove-Item -Path "$tempDir\zoicwareOS.zip" -Force -ErrorAction SilentlyContinue
...
if ($null -eq $folder) {
    ...
    Write-Host 'Make Sure You Have Downloaded Zoicware Correctly by Downloading the ZIP File Here: ' -NoNewline -ForegroundColor Cyan -BackgroundColor Black
    Write-Host 'https://github.com/zoicware/ZOICWARE/releases' -ForegroundColor Yellow -BackgroundColor Black
    Write-Host 'Press any Key to Exit: ' -BackgroundColor Black
...
if (Test-Path "$folder\$currentVersion" -PathType Leaf) {
    ...
    Write-Host "Folder Found But Not Updated at $folder" -BackgroundColor Black
    Write-Host "Make sure you do not have older versions of zoicware on $sysDrive" -BackgroundColor Black
    Write-Host 'Press any Key to Exit: ' -BackgroundColor Black
    $null = $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown')
    exit
}
...
#display neofetch
winfetch 
Write-Host "$([char]27)[40m$([char]27)[37m" -NoNewline 
...
removeallPlans = 0
enableUltimate = 0
enablePowerModeHigh = 0
enablePowerModeBalance = 0 
#OPTIONAL TWEAKS
...
conScanDefender = 0
conCastDevice = 0
conPersonalizeClassic = 0
conRestartExplorer = 0 
conRestartBIOS = 0
conEmptyRecycle = 0
...
if (!(Test-Path -Path $config)) {
    Write-Host 'Creating Config to Track User Settings' -NoNewline -BackgroundColor Black
    Write-Host " [$config]" -ForegroundColor Yellow -BackgroundColor Black
    ...
    foreach ($line in $configContentArray) {
        if ($line -notlike '#*') {
            ...
            if ($settingName[0].trim() -notin $currentConfigNames) { 
                Write-Host "Updating Config with [$($settingName[0].trim())]" -ForegroundColor Yellow -BackgroundColor Black
...
do {
    ...
    $infobutton.Add_Click({
            try {
                ...
                Write-Host 'No Internet Connected...' -ForegroundColor Red -BackgroundColor Black
            }
        })
