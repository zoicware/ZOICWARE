...
'@ 
  ...
  if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
    ...
    if ($extraCBSApps.Checked) {
      ...
      Remove-CBS-Apps
      Write-Status -Message 'Reboot to apply changes. Get Started and Windows Backup will be gone after logon script finishes.' -Type Output
    }
...
defeatMsMpEng
'@
      $script = New-Item "$tempDir\DefeatDefend.ps1" -Value $code -Force
      ...
      New-item -Path "$tempDir\disableReg" -ItemType Directory -Force | Out-Null
      New-Item -Path "$tempDir\disableReg\disable1.reg" -Value $file1 -Force | Out-Null
      New-Item -Path "$tempDir\disableReg\disable2.reg" -Value $file2 -Force | Out-Null
      New-Item -Path "$tempDir\disableReg\disable3.reg" -Value $file3 -Force | Out-Null
      New-Item -Path "$tempDir\disableReg\disable5.reg" -Value $file5 -Force | Out-Null
      New-Item -Path "$tempDir\disableReg\disable6.reg" -Value $file6 -Force | Out-Null
      New-Item -Path "$tempDir\disableReg\disable7.reg" -Value $file7 -Force | Out-Null
      New-Item -Path "$tempDir\disableReg\disable8.reg" -Value $file8 -Force | Out-Null
      New-Item -Path "$tempDir\disableReg\disable9.reg" -Value $file9 -Force | Out-Null
      New-Item -Path "$tempDir\disableReg\disable10.reg" -Value $file10 -Force | Out-Null
      New-Item -Path "$tempDir\disableReg\disable11.reg" -Value $file11 -Force | Out-Null
      $files = (Get-ChildItem -Path "$tempDir\disableReg").FullName
      ...
      Remove-Item "$tempDir\disableReg" -Recurse -Force
      Remove-Item "$tempDir\DefeatDefend.ps1" -Force
...
function import-powerplan {
  ...
  param (
    ...
    , [Parameter(mandatory = $false)] [bool]$enablePowerModeHigh = $false 
    , [Parameter(mandatory = $false)] [bool]$enablePowerModeBalance = $false 
  ...
  $settings['enablePowerModeHigh'] = $checkbox2
  $settings['enablePowerModeBalance'] = $checkbox3
  ...
  if ($Autorun) {
    ...
    $checkbox2.Checked = $enablePowerModeHigh
    $checkbox3.Checked = $enablePowerModeBalance
    ...
    $resetCheckedBttn.Add_Click({
        ...
        Get-AllControls $form | Where-Object { $_ -is [System.Windows.Forms.CheckBox] -or $_ -is [System.Windows.Forms.RadioButton] } | ForEach-Object { 
          $_.Checked = $false 
          $_.ForeColor = 'White'
        }
    ...
    $checkbox2.Text = 'Power Mode High Performance'
    ...
    $checkbox3.Text = 'Power Mode Balanced'
    ...
    foreach ($controller in $controllers.Values) {
      ...
      foreach ($device in $childDevices) {
        ...
        if ($genhubs -and $genhubs.ContainsKey($device.InstanceId)) {
  ...
  if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
    ...
    if ($removeallPlans -or $checkboxALL.Checked) {
      ...
      foreach ($plan in $powerPlans.GetEnumerator()) {
        ...
        powercfg /delete $($plan.Value) *>$null
      }
    }
      ...
      foreach ($line in $regOptions11) {
        ...
        if ($line -like ';*') {
          ...
          if ($name[1].trim() -in $removeTweaks11) {
            ...
            if ($name[1].trim() -eq 'Disable cross device resume') {    
              $Global:resume = $true
            }

            if ($name[1].trim() -eq 'Disable Priority Notifications') {    
              $Global:priorityNoti = $true
            }
...
'@
    ...
    function disable-prioritynotifications {
      #these are whats read by default for the calls and reminders section...not really needed since setting the byte array below also disables
      #but if thats not set then these control those checkboxes plus some hidden ones here...
      Reg.exe add 'HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Notifications\QuietHours\Profiles\Microsoft.QuietHoursProfile.PriorityOnly' /v 'AllowAllCalls' /t REG_DWORD /d '0' /f
      Reg.exe add 'HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Notifications\QuietHours\Profiles\Microsoft.QuietHoursProfile.PriorityOnly' /v 'AllowAllPeople' /t REG_DWORD /d '0' /f
      Reg.exe add 'HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Notifications\QuietHours\Profiles\Microsoft.QuietHoursProfile.PriorityOnly' /v 'AllowAllReminders' /t REG_DWORD /d '0' /f
      Reg.exe add 'HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Notifications\QuietHours\Profiles\Microsoft.QuietHoursProfile.PriorityOnly' /v 'AllowAllTexts' /t REG_DWORD /d '0' /f
      Reg.exe add 'HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Notifications\QuietHours\Profiles\Microsoft.QuietHoursProfile.PriorityOnly' /v 'AllowRepeatCalls' /t REG_DWORD /d '0' /f

  
      function Get-QuietHoursPriorityAppsData {
        $allowedAppsPath = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Notifications\QuietHours\Profiles\Microsoft.QuietHoursProfile.PriorityOnly\AllowedApps'

        $appNames = Get-ChildItem -LiteralPath $allowedAppsPath -ErrorAction SilentlyContinue | Select-Object -ExpandProperty PSChildName

        if (!$appNames -or $appNames.Count -eq 0) {
          #No allowed apps found, hardcode what they most likely are
          $appNames = @(
            ...
            'Windows.System.NearShareExperienceReceive'
          )
        }

        #Fixed 45-byte CB record header 
        $header = [byte[]]@(
          0x43, 0x42, 0x01, 0x00, 0x0A, 0x02, 0x01, 0x00, 0x2A, 0x06, 0xDF, 0xB8, 0xB4, 0xCC, 0x06, 0x2A,
          0x2B, 0x0E, 0xD0, 0x03, 0x43, 0x42, 0x01, 0x00, 0xC2, 0x0A, 0x01, 0xCD, 0x14, 0x06, 0x02, 0x05,
          0x00, 0x00, 0x01, 0x01, 0x02, 0x00, 0x03, 0x01, 0x04, 0x00, 0xCC, 0x32, 0x12
        )

        $footer = [byte[]]@(0x00, 0x00, 0x00, 0x00)

        $body = [System.Collections.Generic.List[byte]]::new()
        $body.Add([byte]$appNames.Count)

        foreach ($name in $appNames) {
          #shouldnt ever be too long
          <#
          if ($name.Length -gt 255) {
            throw "App name '$name' is too long to length-prefix as a single byte."
          }
          #>

          #encode app aumid
          $body.Add([byte]$name.Length)
          $body.AddRange([System.Text.Encoding]::Unicode.GetBytes($name))
        }

        return $header + $body.ToArray() + $footer
      }
      #get the apps that are in the default list of allowed priority apps and encode them into the byte array
      $disablePriorityNotificationsData = Get-QuietHoursPriorityAppsData

      $priorityNotiBasePath = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\CloudStore\Store\DefaultAccount\Current'
      #the path can be like {guid}$windows.data.donotdisturb.quiethoursprofile or default$windows.data.donotdisturb.quiethoursprofile
      $priorityNotiPaths = Get-ChildItem $priorityNotiBasePath -Recurse | Where-Object { $_.PSChildName -like '*microsoft.quiethoursprofile.priorityonly' } 
    
      foreach ($path in $priorityNotiPaths) {
        Set-ItemProperty -LiteralPath $path.PSPath -Name 'Data' -Value $disablePriorityNotificationsData -Type Binary -Force
      }
...
'@
    ...
    if (Test-Path "$tempDir\CustomTweaks.reg") {
      ...
      if ($Global:priorityNoti) {
        disable-prioritynotifications
      }
...
function OptionalTweaks {
  param (
    ...
    , [Parameter(mandatory = $false)] [bool]$conCastDevice = $false
    , [Parameter(mandatory = $false)] [bool]$conRestartExplorer = $false
    , [Parameter(mandatory = $false)] [bool]$conRestartBios = $false
    , [Parameter(mandatory = $false)] [bool]$conEmptyRecycle = $false
    , [Parameter(mandatory = $false)] [bool]$noPasswordLock = $false
  )
  ...
  $checkbox79 = new-object System.Windows.Forms.checkbox
  ...
  $settings['conCastDevice'] = $checkbox75
  $settings['conRestartExplorer'] = $checkbox76
  $settings['conRestartBios'] = $checkbox77
  $settings['conEmptyRecycle'] = $checkbox78
  $settings['noPasswordLock'] = $checkbox79
  ...
  if ($AutoRun) {
    ...
    $checkbox75.Checked = $conCastDevice
    $checkbox76.Checked = $conRestartExplorer
    $checkbox77.Checked = $conRestartBios
    $checkbox78.Checked = $conEmptyRecycle
    $checkbox79.Checked = $noPasswordLock
  }
  else {
    ...
    $miscGroupBox.Size = New-Object System.Drawing.Size(550, 210)
    ...
    $checkbox79.Location = New-Object System.Drawing.Size(10, 170)
    $checkbox79.Size = New-Object System.Drawing.Size(240, 30)
    $checkbox79.Text = 'Remove Password & Lockscreen'
    $checkbox79.ForeColor = 'White'
    $checkbox79.Checked = $false
    #TODO
    $checkbox79.Visible = $false
    $miscGroupBox.Controls.Add($checkbox79)
    ...
    $checkbox75.Location = New-Object System.Drawing.Size(270, 460)
    $checkbox75.Size = New-Object System.Drawing.Size(250, 30)
    $checkbox75.Text = 'Cast to Device'
    $checkbox75.ForeColor = 'White'
    $checkbox75.Checked = $false
    $form.Controls.Add($checkbox75)
    $contextMenuPanel.Controls.Add($checkbox75)

    $checkbox76.Location = New-Object System.Drawing.Size(20, 340)
    $checkbox76.Size = New-Object System.Drawing.Size(250, 30)
    $checkbox76.Text = 'Restart Explorer'
    $checkbox76.ForeColor = 'White'
    $checkbox76.Checked = $false
    $form.Controls.Add($checkbox76)
    $contextMenuPanel.Controls.Add($checkbox76)

    $checkbox77.Location = New-Object System.Drawing.Size(20, 370)
    $checkbox77.Size = New-Object System.Drawing.Size(250, 30)
    $checkbox77.Text = 'Restart to BIOS'
    $checkbox77.ForeColor = 'White'
    $checkbox77.Checked = $false
    $form.Controls.Add($checkbox77)
    $contextMenuPanel.Controls.Add($checkbox77)

    $checkbox78.Location = New-Object System.Drawing.Size(20, 400)
    $checkbox78.Size = New-Object System.Drawing.Size(250, 30)
    $checkbox78.Text = 'Empty Recycle Bin'
    $checkbox78.ForeColor = 'White'
    $checkbox78.Checked = $false
    $form.Controls.Add($checkbox78)
    $contextMenuPanel.Controls.Add($checkbox78)
  ...
  if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
    ...
    if ($checkbox77.Checked) {
      Write-Status -Message 'Adding Restart to BIOS to Context Menu...' -Type Output
      $path = Search-File '*AddRestarttoBIOS.reg'
      Start-Process -filepath "$env:windir\regedit.exe" -Argumentlist "/s $path"
    }
        ...
        $button.Add_Click({
            ...
            try {
              ...
              if ('C:\Program Files\PBOTuner' -notin $exclusionPaths) {
                Add-MpPreference -ExclusionPath 'C:\Program Files\PBOTuner' -Force -ErrorAction SilentlyContinue
    ...
    if ($checkbox76.Checked) {
      Write-Status -Message 'Adding Restart Explorer to Context Menu...' -Type Output
      $path = Search-File '*AddRestartExplorer.reg'
      Start-Process -filepath "$env:windir\regedit.exe" -Argumentlist "/s $path"
    }

    if ($checkbox78.Checked) {
      Write-Status -Message 'Adding Empty Recycle Bin to Context Menu...' -Type Output
      $path = Search-File '*AddEmptyRecycleBin.reg'
      Start-Process -filepath "$env:windir\regedit.exe" -Argumentlist "/s $path"
    }
    ...
    if ($checkbox42.Checked) {
      ...
      Reg.exe delete 'HKCR\*\shellex\ContextMenuHandlers\ModernSharing' /f *>$null
    }
    ...
    if ($checkbox75.Checked) {
      Write-Status -Message 'Removing Cast to Device...' -Type Output
      Reg.exe add 'HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Shell Extensions\Blocked' /v '{7AD84985-87B4-4a16-BE58-8B72A5B390F7}' /t REG_SZ /d ' ' /f *>$null
    }
...
'
    ...
    if ($checkbox61.Checked) {
      ...
      #set for lockscreen and desktop
      $mousePaths = @(
      ...
      switch ($scaling) {
        100 {
          ...
          foreach ($path in $mousePaths) {
            Reg.exe add $path /v 'MouseSensitivity' /t REG_SZ /d '10' /f
            Reg.exe add $path /v 'MouseSpeed' /t REG_SZ /d '0' /f
            Reg.exe add $path /v 'MouseThreshold1' /t REG_SZ /d '0' /f
            Reg.exe add $path /v 'MouseThreshold2' /t REG_SZ /d '0' /f
            Reg.exe delete $path /v 'SmoothMouseXCurve' /f *>$null
            Reg.exe delete $path /v 'SmoothMouseYCurve' /f *>$null
          }
        ...
        125 {
          ...
          foreach ($path in $mousePaths) {
            Set-ItemProperty -Path "registry::$path" -Name 'SmoothMouseXCurve' -Value $SmoothMouseXCurve -Type Binary -Force
            Set-ItemProperty -Path "registry::$path" -Name 'SmoothMouseYCurve' -Value $SmoothMouseYCurve -Type Binary -Force
            Reg.exe add $path /v 'MouseSensitivity' /t REG_SZ /d '10' /f
            Reg.exe add $path /v 'MouseSpeed' /t REG_SZ /d '1' /f
            Reg.exe add $path /v 'MouseThreshold1' /t REG_SZ /d '6' /f
            Reg.exe add $path /v 'MouseThreshold2' /t REG_SZ /d '10' /f
          }
        ...
        175 {
          Write-Status -Message 'Setting Mouse up for 175% Monitor Scaling...' -Type Output
          $SmoothMouseXCurve = [byte[]](
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x60, 0x66, 0x16, 0x00, 0x00, 0x00, 0x00, 0x00,
            0xC0, 0xCC, 0x2C, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x20, 0x33, 0x43, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x80, 0x99, 0x59, 0x00, 0x00, 0x00, 0x00, 0x00
          )
          ...
          Reg.exe add 'HKCU\Control Panel\Desktop' /v 'Win8DpiScaling' /t REG_DWORD /d '0' /f
          Reg.exe add 'HKCU\Control Panel\Desktop' /v 'LogPixels' /t REG_DWORD /d '168' /f
          Reg.exe add 'HKCU\Control Panel\Desktop' /v 'EnablePerProcessSystemDPI' /t REG_DWORD /d '1' /f
        }

        225 {
          Write-Status -Message 'Setting Mouse up for 225% Monitor Scaling...' -Type Output
          $SmoothMouseXCurve = [byte[]](
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0xC0, 0xCC, 0x1C, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x80, 0x99, 0x39, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x40, 0x66, 0x56, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x33, 0x73, 0x00, 0x00, 0x00, 0x00, 0x00
          )
          ...
          Reg.exe add 'HKCU\Control Panel\Desktop' /v 'Win8DpiScaling' /t REG_DWORD /d '0' /f
          Reg.exe add 'HKCU\Control Panel\Desktop' /v 'LogPixels' /t REG_DWORD /d '216' /f
          Reg.exe add 'HKCU\Control Panel\Desktop' /v 'EnablePerProcessSystemDPI' /t REG_DWORD /d '1' /f
        }

        default {
          Write-Status -Message 'Scaling Value Not Supported...Must be 100, 125, 150, 175, 200 or 225' -Type Warning
        }
...
'@
    ...
    if ($checkbox74.Checked) {
      ...
      $psScript = @"
explorer ""$env:APPDATA\Microsoft\Windows\Start Menu\Programs"";explorer ""$env:ProgramData\Microsoft\Windows\Start Menu\Programs""
"@
    ...
    if ($checkbox79.Checked) {
      ...
      #TODO
      Write-Status -Message 'Removing Password...' -Type Output
...
<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  ...
  <Actions Context="Author">
    <Exec>
      ...
      <Arguments>--headless powershell.exe -ep bypass -f `"$scriptPath`"</Arguments>
    </Exec>
  </Actions>
</Task>
"@
...
<Exec>
<Command>conhost.exe</Command>
<Arguments>--headless powershell.exe -ep bypass -f `"$env:programdata\DisableQuickSettingsTiles.ps1`"</Arguments>
</Exec>
</Actions>
</Task>
"@
...
function install-key {
  ...
  if (!(Check-Internet)) {
    ...
    $command = {
      ...
      icacls.exe 'C:\Windows\System32\slmgr.vbs' /grant *S-1-5-32-544:F
