...
'@
    ...
    if ($checkbox3.Checked) {
      ...
      if ($locked) {
        Copy-Item $hostsPath -Destination $env:TEMP -Force
        foreach ($domain in $domains) {
          Add-Content "$env:TEMP\hosts" -Value "0.0.0.0 $domain" -Force
        }
        Remove-Item $hostsPath -force
        Move-Item "$env:TEMP\hosts" -Destination $(Split-Path $hostsPath -Parent) -Force
...
'@
    ...
    if (Test-Path "$env:temp\CustomTweaks.reg") {
      ...
      Remove-Item "$env:USERPROFILE\Desktop\RegTweaks.reg" -Force -ErrorAction SilentlyContinue
      ...
      Set-Content "$env:TEMP\RegTweaks11.reg" -Value $regContent11 -Force | Out-Null
        
      #create full reg tweaks on desktop but run win10 and 11 seperate to avoid some keys not applying
      Set-Content "$env:USERPROFILE\Desktop\RegTweaks.reg" -Value $regContent11 -Force
  ...
  switch ($msgBoxInput) {
    ...
    'OK' {
      ...
      try {
        ...
        Invoke-RestMethod -Uri 'https://download.microsoft.com/download/8/4/A/84A35BF1-DAFE-4AE8-82AF-AD2AE20B6B14/directx_Jun2010_redist.exe' -UseBasicParsing -OutFile "$env:TEMP\DirectXRedist\DXinstaller.exe" -ErrorAction Stop
...
"@
      Set-Content -Path "$env:TEMP\DisableQuickSettings" -Value $content -Force

      schtasks /Create /XML "$env:TEMP\DisableQuickSettings" /TN '\DisableQuickSettingsTiles' /F | Out-Null 

      Remove-Item -Path "$env:TEMP\DisableQuickSettings" -Force -ErrorAction SilentlyContinue
