...
function import-powerplan {
  ...
  if ($Autorun) {
    ...
    $Global:rootHubs = New-Object System.Collections.ArrayList
    $Global:genHubs = New-Object System.Collections.ArrayList

   
    foreach ($usb in $usbDevices) {
      if ($usb.FriendlyName -like '*Hub*') {
        $deviceInfo = @{
          FriendlyName = $usb.FriendlyName
          InstanceId   = $usb.InstanceId
        }
        
        if ($usb.FriendlyName -like '*Root*') {
          $rootHubs.Add($deviceInfo) | Out-Null
        }
        else {
          $genHubs.Add($deviceInfo) | Out-Null
        }
      }
    }
    ...
    # Add root hubs
    foreach ($hub in $rootHubs.GetEnumerator()) {
      $rootNode = New-Object System.Windows.Forms.TreeNode
      $rootNode.Text = $hub.FriendlyName  
      $rootNode.Tag = $hub.InstanceId 
      $treeView.Nodes.Add($rootNode) | Out-Null
    }

    # Add generic hubs under each root hub
    foreach ($hub in $genHubs.GetEnumerator()) {
      foreach ($rootNode in $treeView.Nodes) {
        $childNode = New-Object System.Windows.Forms.TreeNode
        $childNode.Text = $hub.FriendlyName  
        $childNode.Tag = $hub.InstanceId 
  ...
  if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
    ...
    foreach ($rootNode in $treeView.Nodes) {
      if ($rootNode.Checked) {
        Write-Status -Message "Disabling Power Saving For $($rootNode.Text)" -Type Output
        $instanceID = $rootNode.Tag
      ...
      foreach ($childNode in $rootNode.Nodes) {
        if ($childNode.Checked) {
          Write-Status -Message "Disabling Power Saving For $($childNode.Text)" -Type Output
          $instanceID = $childNode.Tag
          Reg.exe add "$($key)\$instanceID\Device Parameters\WDF" /v 'IdleInWorkingState' /t REG_DWORD /d '0' /f
          #uncheck device manager ui
          $powerMgmt = Get-CimInstance MSPower_DeviceEnable -Namespace root\wmi | Where-Object { $_.InstanceName -like "*$instanceID*" }
          $powerMgmt.Enable = $false 
...
function W11Tweaks {
  ...
  if ($Autorun) {
    ...
    $form.Size = New-Object Drawing.Size(450, 660)
    $form.BackColor = 'Black'
    $form.Font = New-Object System.Drawing.Font($dmMonoFont, 8)
    $form.Icon = New-Object System.Drawing.Icon($Global:customIcon)
    ...
    $OKButton = New-Object System.Windows.Forms.Button
    $OKButton.Location = New-Object System.Drawing.Point(112, 580)
    ...
    $CancelButton = New-Object System.Windows.Forms.Button
    $CancelButton.Location = New-Object System.Drawing.Point(210, 580)
